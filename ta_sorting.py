...
sol1 = pd.read_csv('data/test1.csv', header=None).values
sol2 = pd.read_csv('data/test2.csv', header=None).values
sol3 = pd.read_csv('data/test3.csv', header=None).values

def overallocation(sol):
    """ Objective Function: Measure for number of TAs overassigned to sections """
...
def conflicts(sol):
    """ Objective Function: Measures number of time conflicts for TAs"""
...
def undersupport(sol):
    """ Objective Function: Measure for number of sections with insufficient TAs """
...
def unwilling(sol):
    """ Objective Function: Measures number of times a TA is assigned to a section
    they are unwilling to support """
    df_preference = pd.read_csv('data/tas.csv').iloc[:, 3:]
...
def unpreferred(sol):
    """ Objective function: Measures times when tas are assigned to sections marked unpreferred """
...
def unwilling_fix(sols):
    ...
    sol = sols[0]
    sol_copy = sol.copy()
    preferences = pd.read_csv('data/tas.csv').iloc[:, 3:].values
